...
const competitions = [
  {
    ...
    description: 'Our most dominant performance to date. V-3 tore through 8 opponents in the bracket, with zero losses in the group stage. The final lasted just 47 seconds.',
  },
  {
    year: '2024',
  ...
  },
  {
    ...
    robot: 'PAPPU WEHSHI',
    image: '/images/competition.jpeg',
    description: 'Pappu Wehshi was untouchable in the featherweight category — winning all 5 bouts by knockout with an average fight time of under 90 seconds.',
  },
  {
    year: '2022',
    ...
    <div className="bg-black text-white min-h-screen">
          ...
          {years.map((year, yi) => (
            <div key={year} className="flex gap-0 md:gap-12">
              ...
              <div className="hidden md:flex flex-col items-center w-20 shrink-0">
                ...
                <div className="w-px mt-2 flex-1" style={{
                ...
                <div className="space-y-6">
                  {grouped[year].map((comp, ci) => (
                    <motion.div
                      ...
                      className="group border border-gray-800 hover:border-white transition-all duration-500 overflow-hidden"
                    >
                      {/* Image */}
                      <div className="relative w-full overflow-hidden">
                        ...
                        />
                        {/* Result badge */}
                        <span className={`absolute top-4 right-4 text-xs font-semibold tracking-widest uppercase px-3 py-1.5 rounded-full ${
                          comp.resultType === 'winner'
                            ? 'bg-white text-black'
                            : 'bg-black/70 backdrop-blur-sm border border-gray-500 text-white'
                        }`}>
                          {comp.result}
                        </span>
                      </div>
                      ...
                      <div className="p-6 md:p-8">
                        <h3 className="text-xl sm:text-2xl font-bold mb-1">{comp.event}</h3>
                        <p className="text-gray-500 text-xs tracking-wider uppercase mb-4">{comp.location}</p>
                        <p className="text-gray-300 text-sm md:text-base leading-relaxed mb-5">{comp.description}</p>
      ...
      <section className="py-20 md:py-32 px-6 md:px-8 text-center border-t border-gray-800">
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
          ...
          <a href="/sponsors" className="inline-block bg-white text-black px-8 py-3.5 rounded-full text-sm font-semibold hover:bg-gray-200 transition-all duration-300">
            Support the Team
          </a>
        </motion.div>
      </section>
